...
class PageRenderer {
     ...
     *
     * @param array $params An array of available parameters
	 */
    public function addCSS(&$params, \TYPO3\CMS\Core\Page\PageRenderer $pageRenderer) {
    	...
    	$objectManager = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Extbase\\Object\\ObjectManager');
      	...
      	$query = $themeConfigurationRepo->createQuery();
		$query->logicalOr(
			$query->equals('pid', $GLOBALS['TSFE']->rootLine[0]['uid']),
			$query->equals('pid', $GLOBALS['TSFE']->id)
		);
		$themes = $query->execute();

		foreach ($themes as $theme) {
    		$pageRenderer->addCssFile($theme->getCssfile());
		}
	}
}